...
const showGoalToggles = window.SHOW_GOAL_TOGGLES !== false && chartType !== 'pie';
...
function getDataRange(data) {
  const dates = data
    .map(item => item && item.DATE_CREATED ? new Date(item.DATE_CREATED) : null)
...
function getMonthlySales(data) {
  ...
  const hasMonthlyTotals = data.some(item => item && item.year && item.month && item.total_order_qty != null);
  if (hasMonthlyTotals) {
    ...
    return {
      labels: sorted.map(item => `${monthNames[Number(item.month) - 1]} ${item.year}`),
...
const aggregatedSales = getMonthlySales(window.dmlOrderLineMonthlyTotals || window.dmlOrderLineData || []);
const chartData = {
  ...
  const shared = {
    ...
    plugins: {
      legend: {
        display: true,
        labels: {
          color: mode === 'future' ? '#e2e8f0' : '#374151',
          usePointStyle: true,
          padding: 16
        }
      },
      tooltip: {
        backgroundColor: mode === 'future' ? 'rgba(15, 23, 42, 0.96)' : 'rgba(255, 255, 255, 0.96)',
        titleColor: mode === 'future' ? '#f8fafc' : '#111827',
        bodyColor: mode === 'future' ? '#e2e8f0' : '#111827',
        borderColor: mode === 'future' ? '#475569' : '#d1d5db',
        borderWidth: 1,
        padding: 12,
        callbacks: {
          label: context => `${context.dataset.label}: ${context.parsed ?? context.raw}`
        }
      }
    }
  };
  ...
  if (chartType !== 'pie') {
    shared.scales = {
      ...
      y: {
        ...
        grid: {
          color: mode === 'future' ? 'rgba(148, 163, 184, 0.12)' : 'rgba(229, 231, 235, 0.85)'
        },
        ticks: {
          color: mode === 'future' ? '#cbd5e1' : '#6b7280',
          font: { size: 13 },
    ...
    shared.plugins.legend.position = 'bottom';
  }
...
function createChartDatasets(mode) {
  ...
  if (chartType === 'pie') {
    const paletteColors = ['#38bdf8', '#a855f7', '#f97316', '#22c55e', '#facc15', '#fb7185', '#0ea5e9', '#10b981', '#8b5cf6', '#f59e0b'];
    datasets.push({
      label: 'Total Items Sold',
      data: chartData.datasets[0].data,
      backgroundColor: chartData.labels.map((_, index) => paletteColors[index % paletteColors.length]),
      borderColor: '#0f172a',
      borderWidth: 1,
      hoverOffset: 6
    });
    ...
    datasets.push({
      ...
      backgroundColor: chartType === 'line' ? 'rgba(34, 211, 238, 0.18)' : style.backgroundColor,
      hoverBackgroundColor: mode === 'future' ? 'rgba(56, 189, 248, 0.42)' : 'rgba(37, 99, 235, 0.72)'
    });
    ...
    if (mode === 'future' && averageGoalLineEnabled) {
      ...
      const averageValue = values.length ? Math.round(values.reduce((sum, value) => sum + Number(value || 0), 0) / values.length) : 0;
      datasets.push({
        type: 'line',
...
const ctx = document.getElementById('chart_BWBBarChart_chartcan').getContext('2d');
